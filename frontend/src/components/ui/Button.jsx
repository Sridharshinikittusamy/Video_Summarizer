...
export default function Button({
    ...
    const variants = {
        ...
        secondary: "bg-white hover:bg-brand-50 text-brand-700 border-2 border-[#D4A373] shadow-sm font-medium dark:bg-wood-900/40 dark:hover:bg-wood-800/60 dark:text-brand-400 dark:border-brand-500/40",
    ...
    const content = (
        <>
            {loading ? (
                ...
                <>
                    {Icon && <Icon size={size === 'sm' ? 14 : size === 'lg' ? 20 : 18} className={cn(iconClassName)} style={iconStyle} />}
                    {children}
                </>
            )}
        </>
